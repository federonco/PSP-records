...
-- Copiar TODO y hacer clic en Run
-- ============================================
...
-- Añadir columnas si faltan (para DB que ya tenían tablas)
...
-- Políticas (si fallan por "already exists", ejecutá solo las que faltan)
drop policy if exists "locations_read" on psp_locations;
create policy "locations_read" on psp_locations for select to authenticated using (true);

drop policy if exists "locations_read_anon" on psp_locations;
create policy "locations_read_anon" on psp_locations for select to anon using (true);

drop policy if exists "records_read" on psp_records;
create policy "records_read" on psp_records for select to authenticated using (true);

drop policy if exists "records_insert" on psp_records;
create policy "records_insert" on psp_records for insert to authenticated with check (true);

drop policy if exists "records_signoff_update" on psp_records;
create policy "records_signoff_update" on psp_records for update to authenticated
  using (exists (select 1 from psp_admins where user_id = auth.uid()));

drop policy if exists "reports_read" on psp_reports;
create policy "reports_read" on psp_reports for select to authenticated using (true);

drop policy if exists "reports_insert" on psp_reports;
create policy "reports_insert" on psp_reports for insert to authenticated
  with check (exists (select 1 from psp_admins where user_id = auth.uid()));

drop policy if exists "admins_read" on psp_admins;
create policy "admins_read" on psp_admins for select to authenticated using (user_id = auth.uid());

drop policy if exists "sections_read" on psp_sections;
create policy "sections_read" on psp_sections for select to authenticated using (true);

drop policy if exists "sections_read_anon" on psp_sections;
create policy "sections_read_anon" on psp_sections for select to anon using (true);

drop policy if exists "sections_insert" on psp_sections;
create policy "sections_insert" on psp_sections for insert to authenticated with check (true);

drop policy if exists "sections_update" on psp_sections;
create policy "sections_update" on psp_sections for update to authenticated using (true);
